...
Emitted by `SubagentExecutor` (`src/lib/search/subagents/executor.ts`). Nested tool calls are an array field on the subagent's payload (`toolCalls: ToolCallPayload[]`), not nested markup — see the `yaawc-subagent-architecture` skill.
...
- `chatTitle` (`{ chatId, title }`) is a runHost-synthesized wire event, pushed on first-turn completion **after `messageEnd`, before `terminate`** (the only window still reaching subscribers) when auto-title generates a title. The reducer folds it to a `setChatTitle` effect (no message-state mutation); `ChatWindow` updates the open chat's title/tab title and invalidates the sidebar. Auto-title generation itself lives in `attachRunHost`'s `agent_end` branch (gated on the `titleGen` context + first-turn/lock/scheduled checks); see `src/lib/utils/chatTitle.ts`.
