...
const globalStore = globalThis as typeof globalThis & {
  __skillEditApprovalPending?: Map<string, PendingApproval>;
};

const pending =
  globalStore.__skillEditApprovalPending ??
  (globalStore.__skillEditApprovalPending = new Map<string, PendingApproval>());
...
export function waitForApprovalResponse(
  ...
  return new Promise((resolve) => {
    if (pending.size > 100) {
      console.warn(
        `Skill edit approval map has ${pending.size} live entries; check for orphaned approvals.`,
      );
    }
