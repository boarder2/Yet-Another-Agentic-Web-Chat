...
test.describe('approval panel shell', () => {
  ...
  test('a workspace file edit approval', async ({ page, request }) => {
    ...
    await test.step('a workspace file edit renders the shared chrome and names the file', async () => {
    ...
    await test.step('soft approval buttons keep their fill and add a semantic hover border', async () => {
      const reject = panel
    ...
    // Last: this resolves the run, so no cancelAwaitingRun cleanup is needed.
    await test.step('a rejection can be cancelled with Escape before submitting a trimmed reason', async () => {
