...
- Shared UI primitives live in `src/components/ui/` — `Button` (every labeled action button), `Modal` (every dialog; never hand-roll a scrim or use headlessui's `Dialog` directly), `card`, `AppSwitch`, `Select`. See the `design-system` skill. Reuse them rather than hand-rolling markup
