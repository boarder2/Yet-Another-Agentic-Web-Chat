...
export default function UrlsTab({ workspaceId }: { workspaceId: string }) {
  ...
  const ReachIcon = ({ url }: { url: string }) => {
    ...
    if (s === 'checking')
      return <Loader2 size={14} className="animate-spin text-fg/40" />;
    if (s === 'ok')
      return <CheckCircle2 size={14} className="text-green-400" />;
    if (s === 'warn')
      return <AlertCircle size={14} className="text-yellow-400" />;
    return (
      <span className="w-3.5 h-3.5 rounded-full border border-fg/20 inline-block" />
    );
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        <input
          className="flex-1 border border-surface-2 rounded-lg px-3 py-2 text-sm bg-bg focus:outline-none focus:border-accent"
          placeholder="https://…"
          value={draft}
        ...
        <button
          onClick={addUrl}
          className="flex items-center gap-1.5 px-3 py-2 text-sm rounded-lg bg-accent text-white hover:bg-accent/90 transition"
        >
          <Plus size={14} />
          Add
        </button>
            ...
            >
              ...
              <span className="flex items-center gap-1 shrink-0">
                <button
                  onClick={() => move(i, -1)}
                  disabled={i === 0}
                  className="p-1.5 rounded hover:bg-surface-2 disabled:opacity-30 transition"
                  title="Move up"
                >
                  <ChevronUp size={14} />
                </button>
