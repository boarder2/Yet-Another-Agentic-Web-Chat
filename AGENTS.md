...
Agent-authored, self-contained HTML pages that render in a panel beside the chat. `src/lib/artifacts/` (service + the pure `applyExactEdit`, `mention` and CSP modules); tools in `src/lib/tools/agents/artifactTools.ts` (`create_artifact` / `edit_artifact` / `read_artifact`, in the web-search and local-research sets, withheld in private chats since artifacts are durable rows). Every successful write is an immutable full snapshot anchored to the assistant message that produced it. Because a rewind truncates a suffix, surviving versions are always `1..N` with no gaps; `read_artifact` addresses any of them, but nothing restores one — history is browsable, not revertible. Artifacts are agent-authored end to end: no rename, no user editing.

**Ownership** (`artifacts.workspaceId`, `artifact_versions`, app-level cascades) decides everything else. A chat outside a workspace owns its artifacts: the message anchor is a lifecycle link, so a rewind drops that turn's versions and a chat delete drops the lot, with no standalone delete. A chat **inside** a workspace instead creates artifacts the _workspace_ owns — every chat in it can read and edit them (the scope guard is workspace-match, `ArtifactScope`), so the anchor demotes to provenance: rewind and chat delete leave the versions alone and only null out `chatId`. That makes a delete necessary, so workspace artifacts — and only those — have `DELETE /api/artifacts/[id]`, plus a cascade from workspace deletion.

**Referencing across chats.** The prompt roster is what the chat created ∪ what the user mentioned, derived by scanning persisted message text (`roster.ts`) rather than tracked separately, so it can never drift from the transcript. A mention is ordinary markdown — `@[Title](artifact:<id>)`, built and scanned by `mention.ts` — inserted by the composer's `@` popover or the sidebar, rendered as a chip by `MarkdownRenderer`'s `a` override, and resolved server-side against the chat's workspace. The composer's `/skill` and `@artifact` popovers share `useTokenAutocomplete` + `TokenPopover`.

Security rests on the response, not on trusting the content or on how it is embedded: `/api/artifacts/[id]/raw` — the only route that serves artifact bytes as `text/html` — sets a CSP whose `sandbox` directive withholds `allow-same-origin` (opaque origin — no cookies, storage, or credentialed access to this app's API) and whose `default-src 'none'` with no `connect-src` leaves artifact JS no network at all. The sandbox lives on the response rather than only on the viewer's iframe because the same bytes are reachable top-level (open-in-new-tab, popups), where an iframe attribute would not apply. Export injects the network half as a `<meta>` tag, which is best-effort — a meta policy cannot constrain script placed ahead of it. A write emits `artifact_saved`, which both stream writers turn into one `yaawc:artifact` card per artifact per message. UI in `src/components/Artifacts/`: one `ArtifactViewer` under two shells — the chat's side panel, and a workspace artifact's own page (`/workspaces/[id]/artifacts/[artifactId]`), which is where the sidebar's Artifacts list sends you when no chat is mounted to host the panel (`ArtifactBridgeContext` bridges the two, since sidebar and chat are siblings under the workspace layout).
...
- DB schema changes: edit `src/lib/db/schema.ts` only; run `npm run db:generate` to emit the drizzle migration — never hand-write files in `drizzle/` (see `yaawc-db-migrations` skill)
