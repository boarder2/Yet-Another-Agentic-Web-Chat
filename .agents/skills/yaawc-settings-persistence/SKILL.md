...
- Patches **`Storage.prototype.setItem`/`removeItem` once** (NOT the `localStorage` instance — assigning to the exotic instance is silently dropped). Guards `this === window.localStorage` so `sessionStorage` writes never sync. Only allowlisted keys trigger a debounced PATCH (`FLUSH_DELAY_MS`). Debounced persistence remains best-effort and retryable; explicit `flushSettings()` callers receive write failures.
