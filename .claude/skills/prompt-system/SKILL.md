...
## Artifact Guidance & Roster

Two blocks from `src/lib/prompts/simplifiedAgent/artifactGuidance.ts`, both gated on the same `artifactsEnabled` flag that the `webSearch`/`localResearch`/default branches pass to their prompt builders (`!isPrivate`) — so they appear only where the artifact tools do, never in `chat` or `firefoxAI`.

1. **`artifactGuidance`** — static `## Documents (Artifacts)` block: when to create one, the self-contained/no-network contract, and the read-before-edit rule.
2. **`buildArtifactRoster(artifacts, now)`** — `## Documents available in this chat`, one entry per artifact (id, title, current version, relative update time), plus the do-not-duplicate rule; workspace-owned entries are marked and carry a read-before-edit warning, since another chat may have changed them. Rendered only when there are any. The agent self-fetches via `listChatRoster(scope)` at prompt-build time, so resumed, scheduled, and workflow runs get it without threading. The roster is what the chat created ∪ what the user mentioned — mentions are scanned back out of the persisted user messages (`artifacts/roster.ts` + `artifacts/mention.ts`), so the set can't drift from the transcript and a rewind past a mention removes the reference with it. `now` is a parameter to keep the relative times unit-testable (`artifactGuidance.test.ts`).
