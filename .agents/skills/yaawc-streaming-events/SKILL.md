...
| `map_places_discovered` / `map_route_discovered` | Safe provider-grounded discovery milestones that rebuild turn-local place/route handles; transient browser coordinates and exact routes are excluded from durable events                                                               |
| `map_spec` / `map_placement`                     | Provider-validated, persistable map registration plus one immutable writer-owned `yaawc:map` placement per successful composition; unlimited maps per answer, at most 12 unique pins (including route endpoints) and one route per map |
