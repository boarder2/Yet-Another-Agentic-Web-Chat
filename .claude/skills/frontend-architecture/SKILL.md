...
- **`ArtifactCard`** (`src/components/Artifacts/ArtifactCard.tsx`): the in-message re-entry point for an agent-authored HTML document. Props: `id` (artifactId), `title`, `version`, `action`. "Open" calls `openArtifact` from `ArtifactViewerContext`, which `ChatWindow` provides.
- **`ArtifactMention`**: a user's `@[Title](artifact:<id>)` reference. Not a widget envelope — it's ordinary markdown, so `MarkdownRenderer`'s `a` override dispatches it and the parser handles fence/code-span safety. The label comes from the token, so a deleted document still reads correctly (dimmed, inert). The workspace sidebar reaches the panel and composer through `ArtifactBridgeContext`, since it renders outside the chat's tree.
...
| `src/components/Artifacts/ArtifactViewer.tsx`                | The document itself — sandboxed iframe, version switcher, Preview/Source toggle, download; header start/end are slots so the panel and the page can differ                                                           |
| `src/components/Artifacts/ArtifactPanel.tsx`                 | Viewer docked to the viewport's right edge, full-screen below `md`. Its artifact switcher is withheld in a workspace chat, where the sidebar's Documents list is the picker                                          |
...
| `src/components/Artifacts/ArtifactMention.tsx`               | `@[Title](artifact:<id>)` chip, rendered from `MarkdownRenderer`'s `a` override; dimmed and inert once the document is gone                                                                                          |
