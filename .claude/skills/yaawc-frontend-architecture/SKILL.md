...
Note: There is **no Navbar component**. The layout is `Sidebar` (icon rail) + `Layout` (content width container). `Layout.tsx` is a thin content-width wrapper, not a full shell. Its `<main>` reserves the docked artifact panel's width from `--artifact-inset`, so viewport-fixed chat chrome (the `ChatActions` cluster, the composer bar) must shift off the same var rather than pinning to the viewport edge. What's left of `<main>` is the real budget: `WorkspaceShell` measures it and folds its sidebar to the collapsed rail while a panel is docked (presentation only — the stored preference is untouched, and the expand button is withheld rather than left inert), and `useArtifactPanel`'s drag clamp reserves that rail plus the composer's minimum. The `theme/` directory contains `Controller.tsx`, `Provider.tsx`, and `Switcher.tsx`.
...
- **`ArtifactCard`** (`src/components/Artifacts/ArtifactCard.tsx`): the in-message re-entry point for an agent-authored HTML artifact. Props: `id` (artifactId), `title`, `version`, `action`. "Open" calls `openArtifact` from `ArtifactViewerContext`, which `ChatWindow` provides.
- **`ArtifactMention`**: a user's `@[Title](artifact:<id>)` reference. Not a widget envelope — it's ordinary markdown, so `MarkdownRenderer`'s `a` override dispatches it and the parser handles fence/code-span safety. The label comes from the token, so a deleted artifact still reads correctly (dimmed, inert). The workspace sidebar reaches the panel and composer through `ArtifactBridgeContext`, since it renders outside the chat's tree.
...
See the `yaawc-design-system` skill for the token rules these encode.
...
| ↳ open-time scroll                                           | Opening a finished chat anchors at its last answer's first prose block, skipping the `data-execution` widgets it opens with (one-shot per mount, scoped by `data-answer`); live runs follow the stream to the bottom |
...
| `src/components/Artifacts/ArtifactViewer.tsx`                | The artifact itself — sandboxed iframe, version switcher, Preview/Source toggle, download; header start/end are slots so the panel and the page can differ                                                           |
| `src/components/Artifacts/ArtifactPanel.tsx`                 | Viewer docked to the viewport's right edge, full-screen below `md`. Its artifact switcher is withheld in a workspace chat, where the sidebar's Artifacts list is the picker                                          |
| `src/components/Artifacts/ArtifactPage.tsx`                  | Viewer as a standalone page (`/workspaces/[id]/artifacts/[artifactId]`), with delete in place of close                                                                                                               |
| `src/components/Artifacts/ArtifactCard.tsx`                  | In-message artifact card (the `yaawc:artifact` widget)                                                                                                                                                               |
| `src/components/Artifacts/ArtifactMention.tsx`               | `@[Title](artifact:<id>)` chip, rendered from `MarkdownRenderer`'s `a` override; dimmed and inert once the artifact is gone                                                                                          |
| `src/components/TokenPopover.tsx`                            | Completion list shared by the composer's `/skill` and `@document` triggers (`useTokenAutocomplete`)                                                                                                                  |
| `src/components/Artifacts/useArtifactPanel.ts`               | Panel open/active-artifact state + drag-resize width (localStorage, device-local), published as the `--artifact-inset`/`--chat-ml` `:root` vars rather than React state                                              |
