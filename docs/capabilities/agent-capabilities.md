...
By default, every call shows the exact JavaScript and pauses for Run or Deny approval. Settings → Automation can instead enable the instance-wide **Auto-run generated code** preference. Every local off-to-on change requires a dedicated risk confirmation and is active only after it is saved; automatic mode skips per-call approval, but code and results remain visible in the chat and persisted context. The preference applies to normal and private top-level chats and continuable manual workflow chats. It does not make code execution available to Agent Panel executors, deep-research subagents, schedules, or headless workflow runs.

The sandbox reduces risk but is not a perfect isolation boundary: generated JavaScript can use Node built-ins, including `child_process`, and container escapes remain possible. Changing the preference affects subsequent calls only; it neither approves an existing pending request nor stops a container already running. Unreadable or malformed setting state falls back to manual approval. Config, interactive-session, and Docker availability checks still fail closed before either approval mode, and image preparation or execution can still fail afterward.

The global `chart(spec)` helper accepts the simplified chart input, captures chart records privately, and returns short handles only after a clean execution; nonzero, timed-out, or out-of-memory runs register no charts. Docker being configured is not the same as Docker being reachable; the tool checks the daemon when a call is requested. Detailed deployment settings are in the [Configuration](./configuration.md) guide.
