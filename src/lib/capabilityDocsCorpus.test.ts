...
describe('authoritative capability corpus', () => {
  ...
  it('pins the public code-widget contract, limits, and defensive example', () => {
    ...
    for (const snippet of [
      ...
      '`<Chart id="cN"/>`',
      'validates and normalizes every registered input',
      'before the result is cached',
      'normalized canonical specs',
      'canonical-format input are rejected',
      "type: 'bar' | 'line' | 'area';",
      'title: string;',
    ...
    for (const limit of [
      ...
      /Labels per Cartesian chart\s+\|\s+100/,
      /Series per Cartesian chart\s+\|\s+15/,
      /Pie slices per chart\s+\|\s+20/,
      /Bounded chart strings\/colors\s+\|\s+500 characters/,
    ]) {
