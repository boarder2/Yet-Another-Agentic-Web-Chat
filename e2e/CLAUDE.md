...
| `test-chart-create-show`    | Registers a deterministic chart, shows it with the returned turn handle, then answers with fixed text.                                                                                       |
| `test-chart-unknown`        | Tries an unknown chart handle, then recovers with the available current-turn handle before answering.                                                                                        |
| `test-chart-repeat`         | Registers one chart and shows it twice at separate placement events.                                                                                                                         |
| `test-chart-unshown`        | Registers a chart and answers without showing it, proving unshown registrations stay invisible.                                                                                              |
| `test-chart-mention`        | Registers a chart, then narrates the placement in the answer (`show_chart` line plus `{chart_1}`) instead of calling the tool.                                                               |
| `test-chart-raw-tag`        | Streams a legacy raw `<Chart>` tag without registering a chart; new streamed content must strip it.                                                                                          |
| `test-chart-approval`       | Registers a chart, pauses at `ask_user`, then shows the same chart after approval resume.                                                                                                    |
| `test-map-nearby`           | Calls `search_places`, selects its returned `placeHandle` with `show_map`, then answers with a numbered cited summary.                                                                       |
| `test-map-multi`            | Performs several deterministic place searches, selects different handle groupings, and places three independent maps before a prose summary.                                                 |
| `test-map-business`         | Calls `search_places`, identity-bound `get_place_details`, and `show_map`, then answers with the provider-supplied business hours.                                                           |
| `test-map-route`            | Calls `get_route`, selects its returned `routeHandle`, then places the map and answers with a numbered route summary.                                                                        |
| `test-map-location`         | Requests explicit browser-location approval, then routes from the opaque approved location and places a transient-or-saved map.                                                              |
| `test-map-provider-failure` | Returns a deterministic provider failure while preserving a safe no-map answer; no external mapping service is contacted.                                                                    |
...
Mapping integration coverage sets `mappingProvider=test` only through the test settings fixture. `test-map-*` variants use `DeterministicMapProvider`, and browser tile requests are intercepted at `tiles.test`; no real geocoder, places, routing, tile, or LLM service is contacted. Mapping settings are restored after every serial scenario, and location tests use only Playwright's deterministic geolocation context.
...
| `npm run test:e2e:serial` | Run serial specs only   |
...
A spec that mutates instance-wide, DB-synced settings (composer model/panel selection, dashboard widgets, memory toggles, ...) can't just clean up after itself — it also can't tolerate another spec concurrently reading dirty state mid-run. Put specs like that in `e2e/serial/` instead of `e2e/tests/`/`e2e/api/`: that project runs on a single worker, so cross-spec races on shared settings are structurally impossible rather than something each spec has to guard against.
