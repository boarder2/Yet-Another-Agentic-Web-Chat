...
`webSearch.ts` (all tools, iterative research + citations) · `chat.ts` (conversational plus chart lifecycle tools) · `localResearch.ts` (file_search, file citations) · `firefoxAI.ts` (auto-detected, no tools) · `chartingGuidance.ts` (shared `buildChartingGuidance(codeExecutionEnabled)` injected into Chat, Web Search, and Local Research) · `mappingGuidance.ts` (eligible interactive Web Search only). Mapping guidance requires named provider lookups, explicit turn-local handle grouping, numbered prose for every map, normal citations for business facts, unlimited independent writer-owned map placements, per-map route/pin limits, attribution, and omission rather than invention when data is unavailable. Guidance teaches the turn-local `create_chart` → `show_chart` registry flow, simplified input, and optional computed-data `chart(spec)` helper; it does not teach model-authored `<Chart>` tags. Unknown modes fall through to `webSearch` with a console warning.
...
- **Skills** — `buildSkillsPromptSection(modelVisibleSkills)` (`src/lib/skills/promptSection.ts`): `## Available Skills` with `read_skill` instructions; excludes `disableModelInvocation` and already-invoked skills. Explicit `/skill-name` invocations are forced into the current human turn by `buildInvokedSkillsContext()` and persisted for later turns.
...
- **Mapping** — `mappingGuidance` is included only when the runtime actually injects mapping tools into an eligible ordinary Web Search turn. Chat, Local Research, panel, subagent, workflow, schedule, dashboard, Firefox-restricted, and other non-interactive prompts do not receive it. Discovery handles are turn-local; each `show_map` call explicitly composes one immutable placement and may be repeated for other groupings. Saved personalization is a mapping-provider input only after the separate Mapping opt-in; browser coordinates require the location approval flow.
