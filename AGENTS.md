...
- **Mapping** (`src/lib/maps/`): opt-in provider adapters, privacy-aware cache, turn-local place/route handles with explicit unlimited map composition, writer-owned map events/widgets, and explicit browser-location approvals; available only to eligible interactive Web Search turns. Each map keeps the 12-unique-pin and one-route limits.
