...
This skill drives a live dev server for manual/exploratory checks — LLM-backed features need a real provider key. Use the lowest practical test level for automated regression coverage: when coverage needs browser, full-application, or mocked-provider wiring (including chat/agent behavior), use the Playwright e2e suite (`npm run test:e2e`, see `e2e/CLAUDE.md`); otherwise prefer fast unit tests per `AGENTS.md`.
