...
test.describe('history: artifact tab', () => {
  ...
  test('lists artifacts across scopes with provenance, and workspace chips filter the list', async ({
    page,
    request,
  }) => {
    ...
    await test.step('lists artifacts across scopes with provenance', async () => {
      await expect(artifactRow(page, wsDoc)).toBeVisible();
      await expect(artifactRow(page, scopedDoc)).toBeVisible();

      // Workspace chip renders for the owned artifact. The filter chip at
      // the top carries the same text, so scope the assertion to the
      // artifact's own row.
      await expect(
    ...
    await test.step('workspace filter chips filter the list', async () => {
      // Filter to "No workspace" only (chat-scoped).
      await page.getByRole('button', { name: 'No workspace' }).click();
      await expect(artifactRow(page, scopedDoc)).toBeVisible();
      await expect(artifactRow(page, wsDoc)).toBeHidden();
    });
  });
  ...
  test('clicking a row opens the raw artifact in a new tab, and navigates to the chat with ?artifact=', async ({
    page,
    request,
  }) => {
    ...
    await test.step('"open in new tab" affordance opens the raw artifact', async () => {
      const newTabPromise = page.waitForEvent('popup');

      // Scope the click to the seeded artifact's row so we never hit a
      // different row's affordance.
    ...
    await test.step('clicking a row navigates to the chat with ?artifact=', async () => {
