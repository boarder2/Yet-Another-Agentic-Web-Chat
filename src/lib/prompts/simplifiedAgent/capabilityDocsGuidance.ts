...
- For availability questions, request the tool's safe local status and report only \`available\`, \`disabled\`, \`not configured\`, or \`unknown on this device\`. Never expose credentials, endpoints, filesystem paths, provider payloads, or raw configuration.
...
export function buildCapabilityDocsGuidance(): string {
  return capabilityDocsGuidance;
}
