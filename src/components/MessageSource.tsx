...
const MessageSource = ({
  ...
  return oneLiner ? (
    <a
      className={`bg-surface hover:bg-surface-2 transition-colors duration-150 rounded-surface p-2 flex flex-row no-underline items-center space-x-2 font-medium border border-surface-2 focus-border-neutral ${className || ''}`}
      href={source.metadata.url}
      target="_blank"
      style={style}
    >
      {source.metadata.url === 'File' ? (
        <div className="bg-surface-2 hover:bg-surface transition-colors duration-150 flex items-center justify-center w-6 h-6 rounded-pill">
    ...
    <a
      className={`bg-surface hover:bg-surface-2 transition-colors duration-150 rounded-surface p-4 flex flex-row no-underline space-x-3 font-medium border border-surface-2 focus-border-neutral ${className || ''}`}
      href={source.metadata.url}
      target="_blank"
      style={style}
    >
      ...
      <div className="flex flex-col items-center space-y-2 flex-shrink-0">
        {source.metadata.url === 'File' ? (
          <div className="bg-surface-2 hover:bg-surface transition-colors duration-150 flex items-center justify-center w-8 h-8 rounded-pill">
        ...
        <div className="flex flex-row items-center space-x-1 text-fg-subtle text-xs">
          {typeof index === 'number' && (
      ...
      <div className="flex-1 flex flex-col space-y-2">
        ...
        {/* Preview content */}
        <p
          className="text-xs text-fg-muted leading-relaxed overflow-hidden"
          style={{
            display: '-webkit-box',
            WebkitLineClamp: 3,
