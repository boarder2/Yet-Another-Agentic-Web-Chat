...
- **Surface**: `bg-bg`, `bg-surface`, `bg-surface-2`, `bg-well`, `text-fg`, `text-fg-muted` (secondary/helper copy), `text-fg-subtle` (metadata/placeholders/counts), `border-surface-2` (alias `border-border`), `border-border-strong`, `bg-overlay`, `bg-overlay-strong`
- **Brand**: `bg-accent`, `bg-accent-500/600/700`, `bg-accent-soft`, `text-accent`, `text-accent-fg`, `border-accent`, `border-accent-border`
...
- **Motion**: color changes use `transition-colors duration-150`; explicit `duration-100` is reserved for intentionally fast compact-popover feedback. Name non-color transitions (`transition-opacity`, `transition-transform`, `transition-[opacity,transform]`, or `transition-[width]`) and declare their duration. Modal and switch transforms use `duration-200 ease-standard`; Headless UI fades/transforms use the matching explicit property set. CSS props: `--duration-fast/base/slow`, `--ease-standard`. Extra classes: `animate-badge-pop` (one-shot spring entrance), `animate-indeterminate` (progress slide); `animate-spin` for spinners.
- **Typography**: body `text-sm` weight 400; `font-medium` emphasis; `font-semibold` headings (`text-lg/xl/2xl/3xl`); captions `text-xs`; compact counts and preset metadata `text-2xs`.
...
- **`Button`** — every labeled action button. `variant` (`primary` accent fill · `secondary` default · `ghost` · `danger` · `dangerSoft` · `successSoft`), `size` (`sm/md/lg`), `icon`, `loading` (spinner + disable + `aria-busy`). Owns the in-place 1px focus border, disabled, hover — `className` is layout only. Neutral variants focus with accent; filled variants focus with their contrast foreground. Link-as-button: `buttonClasses(variant, size)`. Out of scope: bare icon buttons, tab strips, chips.
...
- **`IconButton`** — every standalone icon action or icon-only link. Requires `label` (used for the accessible name and tooltip), renders a 15px Lucide icon, and owns loading/disabled/`aria-busy` behavior. `tone` (`default` muted foreground · `danger` · `active` · `primary`). Primary and danger use their contrast foreground for focus; default and active use accent. Use `href` for icon links without replacing the link's navigation semantics.
- **`AppSwitch`** — every boolean toggle (headlessui `Switch` styled to tokens): `checked`, `onChange`, `disabled?`, `aria-label`. It keeps a reserved 1px border, uses accent focus while off and the checked contrast foreground while on, with 150ms track-color and 200ms `ease-standard` thumb motion.
- **`Badge`** — standard status and metadata pills: `tone` (`default` · `accent` · `danger` · `success` · `warning` · `info`), standard geometry `rounded-pill px-2 py-0.5 text-xs`, and `size="compact"` for count/preset metadata using `text-2xs`. Use it instead of hand-rolled status pills; keep one-off navigational chips and compact count containers caller-owned.
- **`SettingToggleRow`** — label + optional description + `AppSwitch` row. Owns `text-sm`/`text-xs text-fg-muted` typography, optional `nested` indentation, `mutedLabel`, and an optional icon tile. Use it for settings toggles; callers retain domain callbacks and may pass layout-only `className` for a surrounding surface.
...
- **Maps** — `MapEnvelope`/`MapWidget` keep the numbered semantic list, route summary, links, and attribution outside the Leaflet canvas. The surrounding surface, controls, status states, and focus treatment use YAAWC tokens; provider-controlled tiles are not treated as application palette.
- **Focus** — controls reserve a 1px border and change only its color on `:focus-visible`: use `focus-border-neutral` for accent focus and `focus-border-contrast` for a control's current contrast foreground. Never use a 2px outline or offset for these primitives.
