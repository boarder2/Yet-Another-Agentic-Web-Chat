...
- Shared UI primitives live in `src/components/ui/` — `Button` (every labeled action button; see the `design-system` skill), `card`, `AppSwitch`, `Select`. Reuse them rather than hand-rolling markup
